...
const Tick = ({
    ...
    const sub = PubSub.subscribe(
      ...
      (_, message: PubSubTickMessage) => {
        ...
        if (message.summaryID === summary?.ID) {
          ...
          if (
            message.tick.Distracted === 0 &&
            (timerTick?.Distracted === 0 || timerTick?.Distracted === 1)
          ) {
  ...
  const updateTick = useCallback(
    e => {
      ...
      document.querySelector(`[data-test-id='${testIdAttr}']`).className =
        styles.tictac_clicked;
